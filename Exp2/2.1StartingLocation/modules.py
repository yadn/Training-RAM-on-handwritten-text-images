...
class retina(object):
    ...
    def extract_patch(self, x, l, size):
        ...
        #print("l",l,size)
        coords = self.denormalize(torch.tensor([W,H]), l)
        #print("coords",coords)
        ...
        for i in range(B):
            ...
            #print("******************************************************")
            #print(i,patch_x,patch_x[i],size,from_x,to_x)
            #print("******************************************************")
...
#gb changes***************************************************************************************************
            ...
            #print("ft1",from_x, to_x, from_y, to_y)
            ...
            if self.exceeds(from_x, to_x, from_y, to_y, T):
                ...
                from_y += (size//2+1)
                to_y += (size//2+1)
            #print("ft2",from_x, to_x, from_y, to_y)
            #print("ftdiffFinal", -from_x + to_x, -from_y +to_y)
            # and finally extract
    ...
    def denormalize(self, T, coords):
        ...
        """
	#T=T.cuda()
        scale = torch.tensor([1, 0.5]).cuda()#for x,y
        trans = torch.tensor([0, 1]).cuda()#for x,y
        #print("coords", coords,coords.shape)
        #ret = (scale * ((coords + 1.0) * T.cuda())).long()
        #print(ret,ret.shape)
        return (scale * ((coords + trans) * T.cuda())).long()
    ...
    def exceeds(self, from_x, to_x, from_y, to_y, T):
        ...
        """
        if (
            (from_x < 0) or (from_y < 0) or (to_x > T[0]) or (to_y > T[1])
        ):
            return True
        return False
...
#        print(l_t_prev,"LLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLLL")
        # flatten location vector
        l_t_prev = l_t_prev.view(l_t_prev.size(0), -1)

        # feed phi and l to respective fc layers
        #print("phi type",phi.type())
...
class location_network(nn.Module):
    ...
    def forward(self, h_t):
        # compute mean
        mu1 = F.tanh(self.fc(h_t.detach()))#for y [b,1]
        mu2 = self.fc1(h_t.detach())#for x [b,1]
        #print("mu2",mu2)
        #mu[:,0] = (F.tanh(mu[:,0]+1))*0.5
        # reparametrization trick
        noise1 = torch.zeros_like(mu1)#y
        noise1.data.normal_(std=self.std)#x
        noise2 = torch.zeros_like(mu2)
        noise2.data.normal_(std=(1/4)*self.std)
        l_t1 = mu1 + noise1
        l_t2 = mu2 + noise2
        #print("lt2",l_t2)
        # bound between [-1, 1]
        l_t1 = F.tanh(l_t1)#[b,1]
        l_t2 = F.sigmoid(l_t2)#[b,1]
        mu = torch.cat([mu2,mu1],dim=1)
        l_t = torch.cat([l_t2,l_t1],dim=1)
        #print("lt",l_t)
        #print(l_t.shape, mu.shape)
        return mu, l_t

...
class Trainer(object):
    ...
    def reset(self):
        ...
        h_t = torch.ones(self.batch_size, self.hidden_size)
        ...
        with tqdm(total=self.trainSamplesSize) as pbar:
	        ...
	        while self.train_loader.hasNext():
	            ...
	            for t in range(bmax*onecharglimpse):#self.num_glimpses): #- 1):
	                ...
	                baselines.append(b_t)#as retruned is delta_lt so following is done first before adding delta lt to lt_prev
	                #l_t = torch.tensor([1/21,1]).cuda()*l_t
